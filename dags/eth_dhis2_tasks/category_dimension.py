...
def build_category_dimension(**kwargs):
    """Builds source-specific category dimension (e.g., hmis_categories)."""
    pg_conn_id = kwargs.get("POSTGRES_CONN_ID")
    staging = kwargs["STAGING_SCHEMA_NAME"]
    datasource = kwargs["DATA_SOURCE"].lower() # 'hmis' or 'phem'
    target_table = f"{datasource}_categories"
    
    pg = PostgresHook(postgres_conn_id=pg_conn_id)
    
    with pg.get_conn() as conn:
        with conn.cursor() as cursor:
            # Fetch data specifically for 'category' context within the specific staging schema
            fetch_long_sql = f"""
                SELECT DISTINCT
                ...
                JOIN {staging}.categoryoptions opt ON link.categoryoption_id = opt.id AND opt.combo_type = 'category'
                JOIN {staging}.category_categoryoptions cor ON opt.id = cor.categoryoption_id
                JOIN {staging}.categories cat ON cor.category_id = cat.id AND cat.combo_type = 'category'
            ...
            if not rows:
                logger.warning(f"No category data found in {staging}. Skipping dimension build.")
                return

            # Generate Safe Column Mapping
            ...
            # Update the source-specific mapping table in staging
            map_data = [(k, v) for k, v in mapping.items()]
            execute_values(cursor, f"""
                INSERT INTO {staging}.category_name_map (original_name, safe_name) 
                VALUES %s 
